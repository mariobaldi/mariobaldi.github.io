...
void ordina_merge(abbonato_t el[], int l, int r);
void merge(abbonato_t v[], int s, int c, int d);
...
)
{
   ...
   if(l==r) return;
   ordina_merge(el,l,(l+r)/2);
   ordina_merge(el,(l+r)/2+1,r);
   merge(el,l,(l+r)/2,r);
...
)
{
   ...
   abbonato_t temp[MAXDATI];
   int i=s,j=c+1,k=i;

   while(i<=c && j <= d)
      if(strcmp(v[i].nome,v[j].nome)<0)
	 copia_abbonato(&temp[k++],&v[i++]);
      else
	 copia_abbonato(&temp[k++],&v[j++]);
   while(i<=c) copia_abbonato(&temp[k++],&v[i++]);
   while(j<=d) copia_abbonato(&temp[k++],&v[j++]);
   for(i=s;i<=d;i++) copia_abbonato(&v[i],&temp[i]);
}
